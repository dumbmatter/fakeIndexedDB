...
// Not sure if this is a good way to make things readonly. Messy if other classes need to update a value that is to be presented to the user as readonly, like FDBCursorWithValue.value
    ...
    this._iterate = function (key) {
        ...
        var foundRecord;
        var searchRange = new FDBKeyRange();
        if (this.direction === "next") {
// Start looking at max(key, _position, _range.lower). iterate over records one at a time until one is found that meets the criteria or we exceed _range.higher. Note that all of these things can be undefined, so be graceful.
