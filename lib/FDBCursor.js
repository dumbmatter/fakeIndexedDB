...
function iterateOverCursor(cursor, key) {
    ...
    if (cursor.direction === "next") {
        foundRecord = cursor.source._objectStore.records.find(function (record) {
            if (key !== undefined) {
...
// If range is defined, the record's key is in range.
        ...
        foundRecord = cursor.source._objectStore.records.reverse().find(function (record) {
            if (key !== undefined) {
                if (cmp(record.key, key) !== -1) {
                    return false;
                }
...
// If range is defined, the record's key is in range.
        ...
        cursor.source._objectStore.records.reverse();
