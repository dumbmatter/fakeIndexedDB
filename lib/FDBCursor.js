...
// Not sure if this is a good way to make things readonly. Messy if other classes need to update a value that is to be presented to the user as readonly, like FDBCursorWithValue.value
    ...
    this._iterate = function (key) {
        ...
        if (sourceIsObjectStore) {
            recordStore = this.source._rawObjectStore.recordStore;
        } else {
            recordStore = this.source._rawIndex.recordStore;
        }

        // Constrain the range of keys we need to search over in recordStore. In addition to using this._range, key and this._position can increase the lower bound (for "next") or decrease the upper bound (for "prev").
        var searchRange = new FDBKeyRange(); // Starts with upper and lower undefined, and may remain that way depending on the parameters
        if (this.direction === "next" || this.direction === "nextunique") {
            searchRange.lower = [key, this._position, this._range !== undefined ? this._range.lower : undefined].filter(function (val) {
                return val !== undefined;
            }).sort(cmp)[0];
// Minor possible improvement: when this._position and key are undefined or both are outside this._range, can use lowerOpen from this._range. Same below for the "prev" case
            if (this._range !== undefined) {
                searchRange.upper = this._range.upper;
                searchRange.upperOpen = this._range.upperOpen;
            }
        } else {
            // Same as above, but in the opposite direction
            searchRange.upper = [key, this._position, this._range !== undefined ? this._range.upper : undefined].filter(function (val) {
                return val !== undefined;
            }).sort(function (a, b) {
                return cmp(b, a);
            })[0];
            if (this._range !== undefined) {
                searchRange.lower = this._range.lower;
                searchRange.lowerOpen = this._range.lowerOpen;
            }
        }

        var foundRecord;
        if (this.direction === "next") {
            foundRecord = recordStore.find(searchRange, "next", function (record) {
                if (key !== undefined) {
            ...
            foundRecord = recordStore.find(searchRange, "prev", function (record) {
                if (key !== undefined) {
                    if (cmp(record.key, key) === 1) {
                        return false;
                    }
            ...
            var tempRecord = recordStore.find(searchRange, "prev", function (record) {
                if (key !== undefined) {
                    if (cmp(record.key, key) === 1) {
                        return false;
                    }
            ...
            if (tempRecord) {
                foundRecord = recordStore.get(tempRecord.key);
            }
        }
