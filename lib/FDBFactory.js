...
var DEFAULT_RECORD_STORE_ADAPTER = 'BinarySearchTree';
...
function openDatabase(databases, name, version, request, options, cb) {
    var db;
    if (databases.hasOwnProperty(name)) {
        db = databases[name];
    } else {
        db = new Database({name: name, version: 0, recordStoreAdapter: options.recordStoreAdapter});
        databases[name] = db;
    }
...
module.exports = function (options) {
    ...
    options.recordStoreAdapter = options.recordStoreAdapter !== undefined ? options.recordStoreAdapter : DEFAULT_RECORD_STORE_ADAPTER;
    ...
    this.open = function (name, version) {
        ...
        setImmediate(function () {
            openDatabase(this._databases, name, version, request, options, function (err, connection) {
                var event;
