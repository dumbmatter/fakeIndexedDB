...
// Revert changes
    ...
    this._execRequestAsync = function (source, operation, request) {
        ...
        var setDoneFlagAtEnd = false;
        if (!request) {
            request = new FDBRequest();
            request.source = source;
            request.transaction = source.transaction;
            setDoneFlagAtEnd = true;
        }
        ...
        var cb = function () {
            ...
            try {
                ...
                if (setDoneFlagAtEnd) { request.readyState = 'done'; }
...
//                    this._abort('AbortErrorAAAAA');
                    this._abort(err.name); // Don't seem to make sense, but works
                }
            }

            // If not done, this is a cursor and should be removed becase it'll just be added right back
            if (request.readyState !== 'done') {
                this._requests = this._requests.filter(function (oldRequest) {
